// Command map-virtual-context-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.WithMapNesting(operations.NestingModeFlat),
// using the AWS Durable Execution SDK for Go. Implements the Lambda
// Runtime API directly against the standard library, mirroring
...
// PriceCheckEvent, PriceCheckResult, and MapVirtualContextHandler are
...
func main() {
	...
	durableEntry = durable.WithDurableExecution(MapVirtualContextHandler, &durable.Config{Client: awsClient})
