// Command simple-step-go is a minimal AWS Lambda custom runtime
...
// resolution fails) - unlike the awscli/sigv4lambda stopgaps this
// replaces, which had no equivalent construction step to fail at.
...
func main() {
	...
	// why this replaced the awscli (CLI-shim) and sigv4lambda
	// (hand-rolled signing, had an unresolved bug) stopgaps this example
	// used previously.
	awsClient, err := awssdk.New(context.Background())
	if err != nil {
		log.Fatalf("failed to construct awssdk.Client: %v", err)
	}
	// durableEntry wraps ValidatingHandler (handler.go), not the plain
	// handler - ValidatingHandler is a strict superset (a validation
	// gate ahead of the identical operations.Step call handler itself
	// uses; see that function's own doc for why this is a separate value
	// rather than an in-place edit to handler), so every event this
	// function was previously deployed to accept still produces
	// byte-for-byte the same happy-path result. Switched here
	// specifically so the DEPLOYED function - not just the local test
	// suite - demonstrates docs/ts-sdk-examples-comparison.md gap 6/8's
	// scenario (a handler-level failure before any operation runs) when
	// invoked with an empty "message" field.
	durableEntry = durable.WithDurableExecution(ValidatingHandler, &durable.Config{Client: awsClient})
