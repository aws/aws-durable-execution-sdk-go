// Command map-parallel-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.Map and operations.Parallel, using the AWS
// Durable Execution SDK for Go. Implements the Lambda Runtime API
// directly against the standard library, mirroring this repo's other
// examples' main.go exactly (see any of their doc comments for why: no
// network access to fetch aws-lambda-go or the AWS SDK for Go v2 in this
...
// OrderBatchEvent, OrderBatchResult, and handler are defined in
...
// durableEntry is constructed inside main() rather than at package scope,
// since awssdk.New needs a context and can fail (unlike the old CLI-shim
// client) - see examples/simple-step-go/main.go's identical pattern for
// the same reasoning.
var durableEntry func(ctx context.Context, input types.DurableExecutionInvocationInput) (types.DurableExecutionOutput, error)

func main() {
	entry, err := awssdk.New(context.Background())
	if err != nil {
		log.Fatalf("constructing awssdk client: %v", err)
	}
	durableEntry = durable.WithDurableExecution(handler, &durable.Config{Client: entry})
