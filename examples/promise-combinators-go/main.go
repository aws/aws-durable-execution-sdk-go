// Command promise-combinators-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.All/AllSettled/Any/Race, using the AWS
// Durable Execution SDK for Go. Implements the Lambda Runtime API
// directly against the standard library, mirroring
// examples/simple-step-go/main.go exactly (see that file's doc comment
// for why: no network access to fetch aws-lambda-go or the AWS SDK for
// Go v2 in this development environment).
package main
...
// PriceCheckEvent, PriceCheckResult, and the 4 example handlers
// (AllHandler, AllSettledHandler, AnyHandler, RaceHandler) are defined in
// handler.go, kept separate from this file's Lambda Runtime API plumbing
// so handler_test.go can exercise all 4 via testing.LocalTestRunner
// without needing a real Lambda execution environment. Only AllHandler
// is wired into this deployable binary's own durableEntry, mirroring
// examples/wait-go's own pattern of deploying one primary handler while
// exercising the others only in tests.
...
func main() {
	...
	durableEntry = durable.WithDurableExecution(AllHandler, &durable.Config{Client: awsClient})
