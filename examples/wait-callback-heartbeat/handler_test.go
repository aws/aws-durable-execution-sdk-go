...
func TestHandler(t *testing.T) {
	...
	// (SendDurableExecutionCallbackHeartbeat) which is unavailable in
	// local testing. The submitter step exhausts retries and fails.
	if result.Status != durabletest.Failed {
		t.Fatalf("expected Failed (no real AWS endpoint for heartbeat/callback), got %s", result.Status)
	}
