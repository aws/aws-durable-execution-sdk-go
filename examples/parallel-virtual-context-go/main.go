// Command parallel-virtual-context-go is a minimal AWS Lambda custom
// runtime (provided.al2023-style container image) that runs a durable
// function demonstrating
// operations.WithParallelNesting(operations.NestingModeFlat), using the
// AWS Durable Execution SDK for Go. Implements the Lambda Runtime API
...
// InventoryCheckEvent, InventoryCheckResult, and
// ParallelVirtualContextHandler are defined in handler.go, kept separate
// from this file's Lambda Runtime API plumbing so handler_test.go can
// exercise the handler via testing.LocalTestRunner without needing a
...
func main() {
	...
	durableEntry = durable.WithDurableExecution(ParallelVirtualContextHandler, &durable.Config{Client: awsClient})
