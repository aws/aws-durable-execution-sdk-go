// Command wait-for-callback-multiple-invocations-go is a minimal AWS
// Lambda custom runtime (provided.al2023-style container image) that
// runs a durable function demonstrating checkpoint/replay tracking
// across many suspend/resume cycles, using the AWS Durable Execution
// SDK for Go. Implements the Lambda Runtime API directly against the
// standard library, mirroring examples/wait-for-callback-go/main.go
// exactly (see that file's doc comment for why: no network access to
...
// MultiInvocationEvent, MultiInvocationResult, and handler are defined
// in handler.go, kept separate from this file's Lambda Runtime API
// plumbing so handler_test.go can exercise the handler via
