// Command run-in-child-context-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.RunInChildContext, using the AWS Durable
...
// resolution fails) - unlike the awscli stopgap this replaced, which had
// no equivalent construction step to fail at. This example previously
// used awscli.Client{} (a CLI-shim stopgap client), the last of the six
// original examples still doing so - see
// examples/simple-step-go/main.go's identical fix, and
