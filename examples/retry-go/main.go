// Command retry-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.Step's retry strategies and retry-delay
// suspension, using the AWS Durable Execution SDK for Go. Implements the
// Lambda Runtime API directly against the standard library, mirroring
// this repo's other examples' main.go exactly (see any of their doc
// comments for why: no network access to fetch aws-lambda-go or the AWS
...
// FlakyRequestEvent, FlakyRequestResult, and handler are defined in
