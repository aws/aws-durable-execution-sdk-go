// Command wait-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating operations.Wait, using the AWS Durable Execution SDK for
// Go. Implements the Lambda Runtime API directly against the standard
// library, mirroring examples/simple-step-go/main.go exactly (see that
// file's doc comment for why: no network access to fetch aws-lambda-go
// or the AWS SDK for Go v2 in this development environment).
package main
...
// CoolDownEvent, CoolDownResult, and the 4 example handlers
// (BasicHandler, ConfigurableHandler, NamedHandler, UnawaitedHandler) are
// defined in handler.go, kept separate from this file's Lambda Runtime
// API plumbing so handler_test.go can exercise all 4 via
// testing.LocalTestRunner without needing a real Lambda execution
// environment (see handler.go's doc comments). Only BasicHandler - the
// simplest, canonical operations.Wait usage - is wired into this
// deployable binary's own durableEntry, mirroring
// examples/simple-step-go's own pattern of deploying one primary handler
// while exercising a second, narrower-scope handler (there,
// ValidatingHandler) only in tests.
...
func main() {
	...
	durableEntry = durable.WithDurableExecution(BasicHandler, &durable.Config{Client: awsClient})
