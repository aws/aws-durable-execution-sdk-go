// Command wait-for-callback-timeout-go is a minimal AWS Lambda custom
// runtime (provided.al2023-style container image) that runs a durable
// function demonstrating operations.WithWaitForCallbackTimeout, using
// the AWS Durable Execution SDK for Go. Implements the Lambda Runtime
// API directly against the standard library, mirroring
// examples/wait-for-callback-go/main.go exactly (see that file's doc
// comment for why: no network access to fetch aws-lambda-go or the AWS
// SDK for Go v2 in this development environment).
package main
...
// TimeoutEvent, TimeoutResult, and handler are defined in handler.go,
// kept separate from this file's Lambda Runtime API plumbing so
// handler_test.go can exercise the handler via testing.LocalTestRunner
