...
func TestHandler(t *testing.T) {
	...
	// The handler's "send-heartbeat-and-complete" Step calls the real Lambda
	// API (SendDurableExecutionCallbackHeartbeat + Success) which is
	// unavailable in local testing. The step exhausts retries and fails.
	if result.Status != durabletest.Failed {
		t.Fatalf("expected Failed (no real AWS endpoint for heartbeat step), got %s", result.Status)
	}
