// Command logging-go is a minimal AWS Lambda custom runtime
// (provided.al2023-style container image) that runs a durable function
// demonstrating the SDK's replay-aware, contextual logging
// (pkg/durable/utils/logger.go's ContextLogger), using the AWS Durable
...
// ReportEvent, ReportResult, and handler are defined in handler.go, kept
...
//
// LoggerConfig.ModeAware: true here is technically redundant with
// durable.WithDurableExecution's own default (ModeAware defaults to true
// whenever Config.LoggerConfig is nil at all - see types.LoggerConfig.
// ModeAware's doc) - it is set explicitly anyway so this example's whole
// point (replay-aware log suppression) is visible directly in this
// file's Config literal, rather than relying on a default a reader would
// have to go look up elsewhere to notice is even in effect.
//
