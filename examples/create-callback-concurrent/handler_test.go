...
func TestHandler(t *testing.T) {
	...
	// The handler's "send-callback-N" Steps call the real Lambda API
	// (SendDurableExecutionCallbackSuccess) which is unavailable in local
	// testing. The first step exhausts retries and the execution fails.
	if result.Status != durabletest.Failed {
		t.Fatalf("expected Failed (no real AWS endpoint for send-callback steps), got %s", result.Status)
	}
