...
func TestDurableHandlerInvokeLifecycle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Stack traces carry build file paths, so they are disabled
			// here to keep the exact response comparison stable. The
			// trace content is covered by the stack trace tests.
			h := Wrap(tt.handler, withLambdaAPI(&fakeLambda{}), WithStackTraces(false))
