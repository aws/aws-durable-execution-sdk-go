...
// to [Dag], not to a task registration. A negative value is a configuration
// error. When this option is not set, the DAG defaults to
// [DefaultDagMaxConcurrency] (40) top-level tasks rather than running
// unbounded; pass an explicit 0 to opt back into unbounded top-level
// concurrency. To bound the inner fan-out of a Map/Parallel task, use
