// Command map_op_serde_replay implements conformance requirement 9-20: Map with
// an operation-level serdes — deserialize on replay (wait after the map).
package main
...
func (opSerdes) Unmarshal(_ context.Context, _ durable.SerdesContext, data []byte, v any) error {
	...
	if !ok {
		return fmt.Errorf("unexpected type %T, want *durable.BatchResult[string]", v)
	}
...
func handler(ctx durable.Context, _ any) ([]string, error) {
	items := []string{"x", "y"}
	result, err := durable.Map(ctx, "op-serde-replay", items, func(_ durable.Context, item string, _ int) (string, error) {
